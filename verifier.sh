...
if [ "\$GEM_FIREFOX_ON_HOLD" ]; then
    ...
    if [ "\$ffox_cur" != "\$ffox_can" ]; then
        echo "WARNING: firefox is upgraded, run it to accomplish update operations"
...
_devtest_innervm_run () {
    ...
    ssh -t  $lxc_ip "sudo add-apt-repository -y ppa:openquake/ppa"
    ssh -t  $lxc_ip "sudo apt-get update"
    ssh -t  $lxc_ip "sudo apt-get install -y python-decorator python-h5py python-psutil python-concurrent.futures python-oq-hazardlib python-oq-risklib"
