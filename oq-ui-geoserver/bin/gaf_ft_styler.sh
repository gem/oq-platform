...
IFS='
'
for i in $(sed  -n '7p;7,22p' data/cpt/GMT_no_green.cpt) ; do
    r[$ct]="$(printf "%02X" $(echo "$i" | awk '{ print $2; }'))"
    g[$ct]="$(printf "%02X" $(echo "$i" | awk '{ print $3; }'))"
    b[$ct]="$(printf "%02X" $(echo "$i" | awk '{ print $4; }'))"
    ct=$((ct + 1))
done
echo "CT1: $ct"

ct=0
#for i in  0.0 0.22584577 0.50269786 0.84207582 1.25810086 1.7680834 2.39324334 3.15959301 4.09901951 5.25061152 6.66228572 8.39278057 10.51410037 13.11451128 16.30221401 20.20984593 30.0; do
for i in  0.0 0.225 0.502 0.842 1.258 1.768 2.393 3.159 4.099 5.250 6.662 8.392 10.514 13.114 16.302 20.209 30.0; do
    v[$ct]=$i
    ct=$((ct + 1))
done
echo "CT2: $ct"
...
cat <<EOF
<Rule>
       <Name>${v[$i]}&lt;= net_slip_rate &lt; ${v[$ipo]}</Name>
       <Title>${v[$i]}&lt;= net_slip_rate &lt; ${v[$ipo]}</Title>
       <ogc:Filter>
         <ogc:And>           
           <ogc:PropertyIsGreaterThanOrEqualTo>
               <ogc:PropertyName>ns_net_slip_rate_comp</ogc:PropertyName>
               <ogc:Literal>${v[$i]}</ogc:Literal>
               </ogc:PropertyIsGreaterThanOrEqualTo>
           
           <ogc:PropertyIsLessThan>
               <ogc:PropertyName>ns_net_slip_rate_comp</ogc:PropertyName>
               <ogc:Literal>${v[$ipo]}</ogc:Literal>
               </ogc:PropertyIsLessThan>
         </ogc:And>
       </ogc:Filter>
          <LineSymbolizer>
            <Stroke>
              <CssParameter name="stroke">#${r[$i]}${g[$i]}${b[$i]}</CssParameter>
            </Stroke>
          </LineSymbolizer>
     </Rule>
EOF
...
cpt2hexval () {
    ...
    grep -q '#[ 	]*COLOR_MODEL[ 	]=' $fname
    if [ $? -eq 0 ]; then
        grep -q '#[ 	]*COLOR_MODEL[ 	]=[ 	]RGB' $fname
        if [ $? -ne 0 ]; then
            return 1
        fi
    fi
