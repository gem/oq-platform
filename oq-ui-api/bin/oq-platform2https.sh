...
export GEM_GEOSERVER_GLOBAL="/var/lib/geoserver/geonode-data/global.xml"
export TB='	'
export NL='
'
...
if [ "$1" = "revert" ]; then
    ...
    for cfg in "$GEM_GN_LOCSET" "$GEM_APACHE_CONF" "$GEM_TOMCAT_CONF" "$GEM_GEOSERVER_CONFIG" "$GEM_GEOSERVER_GLOBAL"; do
...
#
# tomcat configuration file fix
xmlstarlet ed -P -S -L \
    -i "//Connector[@protocol='HTTP/1.1']" -t attr -n scheme -v 'https' \
    -a "//Connector[@protocol='HTTP/1.1']" -t attr -n proxyName -v "$SITEHOST" \
    -a "//Connector[@protocol='HTTP/1.1']" -t attr -n proxyPort -v "443" \
    -d "//Connector[@protocol='HTTP/1.1']/@redirectPort" "${GEM_TOMCAT_CONF}"
