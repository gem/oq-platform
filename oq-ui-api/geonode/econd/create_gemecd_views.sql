...
photologue_photo.id AS photoid, photologue_photo.qualitycode, photologue_photo.name AS photofilename, inventoryclass.id AS inventoryclassid, 
inventoryclass.name AS inventoryclassname, unifieddamagelevel.name AS unifieddamagelevelname, unifieddamagelevel.id AS unifieddamagelevelid, 
lookupassetclass.name AS assetclass, lookupassetclass.id AS assetclassid, count(surveyvalue.id) AS aggregationtest
  ...
  GROUP BY event.name, event.id, event.country, event.yearint, event.the_geom, study.name, study.id, location.id, location.name, location.the_geom, 
  location.isaggregated, photologue_photo.id, photologue_photo.name, photologue_photo.qualitycode, inventoryclass.name, inventoryclass.id, 
  unifieddamagelevel.name, unifieddamagelevel.id, lookupassetclass.name, lookupassetclass.id;  
...
CREATE OR REPLACE VIEW gemecdevents_quick AS 
 SELECT event.name AS eventname, event.id AS eventid, event.country, event.yearint, event.partner
   FROM event
