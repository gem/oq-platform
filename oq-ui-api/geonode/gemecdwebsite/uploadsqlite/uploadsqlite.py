...
from econd.sqliteimport import SQLiteImporter

template_name = "uploadsqlite/templates/uploadsqlite.html"

class UploadSQLite (Pagebase):

    class UploadForm(forms.Form):
        sqlitefile = forms.FileField(label='Upload SQLite file', required=True)

    def dispatch(self, request, *args, **kwargs):
        self.preProcessPage(request, **kwargs)
        self.page_context['page_title'] = 'Upload SQLite data file'
        ...
        if request.method == 'POST':
            ...
            if uploadform.is_valid():
                the_file = request.FILES['uploadform-sqlitefile']
